...
var HelloWorldLayer = cc.Layer.extend({
    ...
    ctor:function () {
        ...
        for(var k in tileMapProperties){
            cc.log(k +":"+ tileMapProperties[k]);
        }
        cc.log("tileMap的MapP1属性："+tileMap.getProperty("MapP1"));
        ...
        cc.log("tiles.length:"+tiles.length);//所有瓦片数量
        var tileAt = bgLayer.getTileAt(cc.p(1,1));//tileMap坐标系下得到对应的精灵
        tileAt.setColor(cc.color.BLUE);
        var tileGIDAt = bgLayer.getTileGIDAt(cc.p(6,6));//这里指的是TileMap坐标系
        ...
        sp.setScale(32/sp.getContentSize().width,32/sp.getContentSize().height)
        this.addChild(sp);
        ...
        function moveAndCollid(pos){
            ...
            if(properties == null){
                ...
                sp.setPosition(newPosition);
                cc.log("吃了一个西瓜"+tileX+"_"+tileY);
                bgLayer.getTileAt(cc.p(tileX,tileY)).setVisible(false);
                // bgLayer.getTileAt(cc.p(tileX,tileY)).removeFromParent(true);
                bgLayer.getTileAt(cc.p(tileX,tileY)).setPosition(0,0);
                 // bgLayer.getTileAt(cc.p(tileX,tileY)).setColor(cc.color.RED);
            }else{
                sp.setPosition(newPosition);
            }
        }
